...
def process_html_with_soup(html: str, tag: str, attributes: dict,
                           find_all: bool) -> Union[ResultSet, BeautifulSoup, None]:
    try:
...
def found_us_dot(td_elements: BeautifulSoup) -> int:
    extracted_us_dot = -1
...
def get_info_list(all_info: BeautifulSoup) -> Optional[list]:
    extracted_info_list = []
...
def get_today_mc_register_menu() -> Optional[ResultSet]:
    params = {
        "pv_choice": "FED_REG",
        "x": "9",
        "y": "14",
...
def get_mc_id_user_info(us_dot: int) -> Optional[BeautifulSoup]:
    params = {
        "SearchType": "DOT",
        "DOTNumber": us_dot,
        "Name": "",
...
def save_info_in_file(saved_file: IO, user_info: list, mc_id: str):
    print(f"Downloading Info From MC_ID {mc_id}")
    saved_file.write(str(user_info) + "\n")
    saved_file.flush()


def get_user_info_list(saved_file: IO, mc_number_list: list):
    for mc_number in mc_number_list:
        ...
        if info_list is None:
            ...
            continue
        save_info_in_file(saved_file, info_list, mc_number)


def main(saved_file: IO):
